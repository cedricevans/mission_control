...
import { getPublisherInvoices } from '@/lib/storage';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Download, Eye } from 'lucide-react';
import { useToast } from '@/components/ui/use-toast';

function PublisherInvoice() {
  const { toast } = useToast();
  const invoices = getPublisherInvoices();
  ...
  return (
     <div className="space-y-6">
      <h1 className="text-3xl font-display font-bold">Publisher Invoices</h1>
              ...
              <tr key={invoice.id} className="border-b border-[var(--border-color)] last:border-0 hover:bg-[var(--hover-bg)]">
                ...
                <td className="p-4">
                  <Badge className={
                    ...
                    invoice.status === 'Disputed' ? 'bg-red-500 text-white' :
